...
const QuestionRecheck = () => {
  ...
  const getDifficultyColor = (level: number) => {
    switch (level) {
      case 1: return 'bg-success/15 dark:bg-success/30 dark:text-white text-text  border border-success/30 dark:border-success/50'
      case 2: return 'bg-info/15 dark:bg-info/30 dark:text-white text-text  border border-info/30 dark:border-info/50'
      case 3: return 'bg-warning/15 dark:bg-warning/30 dark:text-white text-text  border border-warning/30 dark:border-warning/50'
      default: return 'bg-muted dark:bg-muted/50 text-text dark:text-muted-foreground border border-border dark:border-border'
    }
  }

  const getDifficultyText = (level: number) => {
    switch (level) {
      case 1: return t('table.difficultyEasy')
      case 2: return t('table.difficultyMedium')
      case 3: return t('table.difficultyHard')
      default: return t('table.difficultyUnknown')
    }
  }

  const getQuestionTypeColor = (type: string) => {
    if (!type) return 'bg-[#3d3a3b]/20 dark:bg-[#3d3a3b]/40 text-text dark:text-muted-foreground border border-border/30 dark:border-border/60';
  ...
  const groupedQuestions: GroupedQuestions = unconfirmedQuestions.reduce((acc: GroupedQuestions, question: Question) => {
  ...
  const calculateCourseDifficulty = (courseGroup: CourseGroup) => {
    ...
    return totalQuestions > 0 ? Math.round(totalDifficulty / totalQuestions) : 0;
  }
  ...
  const calculateChapterDifficulty = (chapterGroup: ChapterGroup) => {
    ...
    const totalDifficulty = chapterGroup.questions.reduce((sum, q) => sum + q.difficultyLevel, 0);
  ...
  if (initialLoading) {
    return (
      <div className="min-h-screen bg-bg dark:bg-gray-900 flex items-center justify-center p-6" dir="rtl">
  ...
  return (
    <div className="min-h-screen bg-bg dark:bg-gray-900 p-6" dir="rtl">
      <div className="max-w-7xl mx-auto space-y-8">
        {/* Header Section */}
        <div className="bg-card-bg dark:bg-gray-800 border border-border dark:border-gray-700 shadow-sm dark:shadow-gray-900/50 rounded-xl p-6 transition-all duration-200 hover:shadow-l dark:hover:shadow-gray-800/50">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-accent/20 dark:bg-accent/30 rounded-lg">
                <BookOpen className="w-6 h-6 text-text dark:text-white" />
              </div>
              <div>
                <h1 className="text-2xl sm:text-3xl font-bold text-text dark:text-white font-arabic">
                  {t('title')}
                </h1>
                <p className="text-text-secondary dark:text-gray-400 mt-1 font-arabic">
                  {t('description')}
                </p>
              </div>
            </div>
        ...
        <div className="bg-card-bg dark:bg-gray-800 border border-border dark:border-gray-700 shadow-md dark:shadow-gray-900/50 rounded-xl overflow-hidden transition-all duration-200">
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {Object.values(groupedQuestions).map((courseGroup: CourseGroup) => {
                  const isCourseExpanded = expandedCourses.has(courseGroup.course.id)
                  ...
                  const courseDifficulty = calculateCourseDifficulty(courseGroup)
                  ...
                  return (
                    <React.Fragment key={courseGroup.course.id}>
                      ...
                      <TableRow className="bg-bg dark:bg-gray-900 hover:bg-bg/80 dark:hover:bg-gray-800/80 transition-colors duration-150">
                        <TableCell>
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => toggleCourse(courseGroup.course.id)}
                            disabled={expandAllLoading}
                        ...
                        <TableCell>
                          <div className="flex items-center gap-3">
                            <div className="p-2 bg-accent/20 dark:bg-accent/30 rounded-lg">
                              <BookOpen className="w-4 h-4 text-text dark:text-white" />
                            </div>
                            <div>
                              <div className="font-semibold text-text dark:text-white font-arabic">
                                {courseGroup.course.name || t('table.noCourseName')}
                              </div>
                              <div className="text-sm text-text-secondary/70 dark:text-gray-400 font-arabic">
                                {t('table.courseNumber')}: {courseGroup.course.id}
                              </div>
                            </div>
                          </div>
                        </TableCell>
                        <TableCell>
                          <div className="font-semibold text-text dark:text-white">{totalCourseQuestions}</div>
                          <div className="text-sm text-text-secondary/70 dark:text-gray-400 font-arabic">
                            {courseChapters.length} {t('table.chapterCount')}
                          </div>
                        </TableCell>
                        <TableCell>
                          <Badge className={`font-arabic dark:text-white ${getDifficultyColor(courseDifficulty)}`}>
                            {getDifficultyText(courseDifficulty)}
                          </Badge>
                        </TableCell>
                        <TableCell>
                          <div className="text-sm text-text-secondary dark:text-gray-400 font-arabic">
                            {courseChapters.length > 0 && courseChapters[0].questions.length > 0
                              ? courseChapters[0].questions[0].questionType?.name || t('table.questionTypeUnknown')
                              : t('table.noQuestions')
                            }
                          </div>
                        </TableCell>
                      ...
                      {isCourseExpanded && courseChapters.map((chapterGroup: ChapterGroup) => {
                        const isChapterExpanded = expandedChapters.has(chapterGroup.chapter.id)
                        const chapterDifficulty = calculateChapterDifficulty(chapterGroup)
                        ...
                        return (
                          <React.Fragment key={chapterGroup.chapter.id}>
                            ...
                            <TableRow className="bg-card-bg dark:bg-gray-800 hover:bg-bg/50 dark:hover:bg-gray-700/50">
                              ...
                              <TableCell>
                                <div className="font-medium text-text dark:text-white">{chapterGroup.questions.length}</div>
                              </TableCell>
                              <TableCell>
                                <Badge className={`font-arabic dark:text-white ${getDifficultyColor(chapterDifficulty)}`}>
                              ...
                              <TableCell>
                                <div className="text-sm text-text-secondary dark:text-gray-400 font-arabic">
                                  {chapterGroup.questions.length > 0
                                    ? chapterGroup.questions[0].questionType?.name || t('table.questionTypeUnknown')
                                    : t('table.noQuestions')
                                  }
                                </div>
                              </TableCell>
                            ...
                            {isChapterExpanded && chapterGroup.questions.map((question: Question, questionIndex: number) => (
                              <TableRow key={question.id} className="hover:bg-bg/30 dark:hover:bg-gray-800/30 transition-colors duration-150">
                                <TableCell></TableCell>
                                <TableCell>
                                  <div className="pr-16">
                                    <div className="font-medium text-text dark:text-white mb-1 font-arabic">
                                      س{questionIndex + 1}: {question.questionText.substring(0, 100)}
                                      {question.questionText.length > 100 ? '...' : ''}
                                    </div>
                                    {question.notes && (
                                      <div className="text-sm text-[#4a151e] dark:text-red-300 mt-1 font-arabic">
                                        <span className="font-medium">{t('table.note')}</span> {question.notes.substring(0, 50)}
                                    ...
                                    {question.questionOptions && question.questionOptions.length > 0 && (
                                      <div className="text-sm text-text-secondary/70 dark:text-gray-500 mt-1 font-arabic">
                                        {question.questionOptions.length} {t('table.optionsCount')}
                                      </div>
                                    )}
                                  </div>
                                </TableCell>
                                <TableCell>
                                  <div className="text-sm dark:text-white text-text-secondary/70 font-arabic">
                                    {t('table.questionId')}: {question.id}
                                  </div>
                                </TableCell>
                                <TableCell>
                                  <Badge className={`font-arabic dark:text-white ${getDifficultyColor(question.difficultyLevel)}`}>
                                    {getDifficultyText(question.difficultyLevel)}
                                  </Badge>
                                </TableCell>
                                <TableCell>
                                  <Badge className={`font-arabic ${getQuestionTypeColor(question.questionType?.name || '')}`}>
                                ...
                                <TableCell>
                                  <div className="flex items-center gap-1 font-arabic">
                                    <Clock className="w-4 h-4" />
                                    <span className="text-text dark:text-white">{question.timeLimit || 180} {t('table.seconds')}</span>
                                  </div>
                                </TableCell>
                                <TableCell>
                                  <div className="flex gap-2">
                                    <Button
                                      ...
                                      disabled={confirmLoading === question.id || deleteLoading === question.id}
                                          ...
                                          <AlertDialogDescription className="text-text-secondary dark:text-gray-400 font-arabic">
                                            ...
                                            {question.questionText && (
                                              <span className="mt-2 p-2 block bg-bg dark:bg-gray-900 rounded text-sm text-text dark:text-gray-300">
                                                {t('deleteDialog.questionPreview')} {question.questionText.substring(0, 100)}
                                                {question.questionText.length > 100 ? '...' : ''}
                                              </span>
                                            )}
                                          </AlertDialogDescription>

...
import {
    ChevronLeft, Plus, Trash2, Save, FileText, BookOpen, Award, Settings, RefreshCw, Loader2, AlertCircle, CheckCircle2, XCircle, BarChart3
} from "lucide-react";
...
const QUESTION_LEVELS = [
    { id: 1, name: "سهل", color: "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-700" },
    { id: 2, name: "متوسط", color: "bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-300 border border-amber-300 dark:border-amber-700" },
    { id: 3, name: "صعب", color: "bg-rose-100 dark:bg-rose-900/30 text-rose-800 dark:text-rose-300 border border-rose-300 dark:border-rose-700" },
];
...
function distributeQuestions<T extends { count: number; percentage: number }>(
    ...
    return items.map((item, index) => ({
        ...
        percentage: ((equalCount + (index < remainder ? 1 : 0)) / totalQuestions) * 100
    }));
}
...
export default function ExamTemplatePage() {
    ...
    const bank = useSelector((state: RootState) => state.supervisor.courseBankDetails);
    ...
    const fetchBankDetails = useCallback(async () => {
        ...
        try {
            ...
            toast.error(error instanceof Error ? error.message : t('errors.loadBankFailed'));
        } finally {
            setLoading(false);
        }
    }, [bankId, dispatch, t]);
                ...
                chapter.questionsLevels.forEach(level => {
                    if (!levelCounts[level.id]) {
                        ...
                        levelCounts[level.id] = {
                            name: level.title,
                            count: 0,
                            color: levelInfo?.color || "bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-300 border border-gray-300 dark:border-gray-700"
                        };
                    }
    ...
    const validateTemplate = useCallback(() => {
        ...
        const chaptersTotal = formData.chapterSettings.reduce((sum, setting) => sum + setting.count, 0);
        ...
        if (formData.levelsSettings.length > 0) {
            const levelsTotal = formData.levelsSettings.reduce((sum, setting) => sum + setting.count, 0);
            ...
            formData.levelsSettings.forEach(setting => {
                ...
                if (setting.count > availableCount) {
                    const levelName = QUESTION_LEVELS.find(l => l.id === setting.level)?.name || `المستوى ${setting.level}`;
                    errors.push(
        ...
        if (formData.typeSettings.length > 0) {
            const typesTotal = formData.typeSettings.reduce((sum, setting) => sum + setting.count, 0);
            ...
            formData.typeSettings.forEach(setting => {
                ...
                if (setting.count > availableCount) {
                    const typeName = availableQuestionTypes.find(t => t.id === setting.questionTypeId)?.name || `النوع ${setting.questionTypeId}`;
                    errors.push(
        ...
        return errors;
    }, [formData, availableChapters, levelStats, typeStats, availableQuestionTypes, t]);
    ...
    const handleAddChapterSetting = (chapterId: number) => {
        ...
        const distributedChapters = distributeQuestions(newChapterSettings, formData.questionsCount);
    ...
    const handleRemoveChapterSetting = (chapterId: number) => {
        ...
        const remainingChapters = formData.chapterSettings.filter(setting => setting.chapterId !== chapterId);
        const distributedChapters = distributeQuestions(remainingChapters, formData.questionsCount);
        ...
        setFormData(prev => ({
            ...
            chapterSettings: prev.chapterSettings.map(setting =>
                setting.chapterId === chapterId
                    ? { ...setting, count: validCount, percentage: (validCount / prev.questionsCount) * 100 }
                    : setting
            )
        }));
    };
    ...
    const handleAddLevelSetting = (level: number) => {
        ...
        const distributedLevels = distributeQuestions(newLevelSettings, formData.questionsCount);
    ...
    const handleRemoveLevelSetting = (level: number) => {
        const remainingLevels = formData.levelsSettings.filter(setting => setting.level !== level);
        const distributedLevels = distributeQuestions(remainingLevels, formData.questionsCount);
    ...
    const handleAddTypeSetting = (questionTypeId: number) => {
        ...
        const distributedTypes = distributeQuestions(newTypeSettings, formData.questionsCount);
    ...
    const handleRemoveTypeSetting = (questionTypeId: number) => {
        const remainingTypes = formData.typeSettings.filter(setting => setting.questionTypeId !== questionTypeId);
        const distributedTypes = distributeQuestions(remainingTypes, formData.questionsCount);
    ...
    const handleUpdateTotalQuestions = (total: number) => {
        ...
        if (total > maxAvailableQuestions) {
            toast.error(t('errors.questionsExceedAvailable', { count: maxAvailableQuestions }));
            return;
        }

        // Update chapter counts proportionally
        const updatedChapters = distributeQuestions(formData.chapterSettings, total);
    ...
    }, [formData.chapterSettings, formData.levelsSettings, formData.typeSettings, formData.questionsCount, validateTemplate]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            toast.error(error instanceof Error ? error.message : t('errors.saveTemplateFailed'));
        } finally {
            setIsSaving(false);
        }
    };
    ...
    const calculateDistributionSummary = () => {
        const summary = {
            chaptersTotal: formData.chapterSettings.reduce((sum, s) => sum + s.count, 0),
        ...
        if (summary.chaptersTotal !== formData.questionsCount) summary.isBalanced = false;
        if (formData.levelsSettings.length > 0 && summary.levelsTotal !== formData.questionsCount) summary.isBalanced = false;
        if (formData.typeSettings.length > 0 && summary.typesTotal !== formData.questionsCount) summary.isBalanced = false;
    ...
    if (loading) {
        return (
            <div className="min-h-screen bg-linear-to-b from-gray-50 to-white dark:from-gray-950 dark:to-gray-900 flex items-center justify-center" dir="rtl">
    ...
    return (
        <div className="min-h-screen bg-linear-to-br from-gray-50 via-white to-gray-100 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950 p-4 md:p-6 overflow-x-hidden" dir="rtl">
            <div className="max-w-7xl mx-auto">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-gray-600 dark:text-gray-400 mt-1">
                                    {bank?.code} • {bank?.course} • {bank?.questionsCount} {t('questionsAvailable')}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="mb-6 p-4 rounded-lg border border-rose-300 dark:border-rose-700 bg-rose-50/50 dark:bg-rose-900/10 backdrop-blur-sm">
                            <div className="flex items-start gap-3">
                                ...
                                <div className="space-y-2 flex-1">
                                    <h4 className="font-medium text-rose-700 dark:text-rose-300">{t('validationErrors')}</h4>
                                    <ul className="space-y-1">
                                        {validationErrors.slice(0, 3).map((error, index) => (
                                            <li key={index} className="text-sm text-rose-700 dark:text-rose-300 flex items-center gap-2">
                                        ...
                                        {validationErrors.length > 3 && (
                                            <li className="text-sm text-rose-700 dark:text-rose-300">
                                                {t('andMoreWarnings', { count: validationErrors.length - 3 })}
                                            </li>
                                        )}
                                    </ul>
                                </div>
                ...
                <div className="space-y-6">
                        ...
                        <CardHeader className="border-b border-gray-100 dark:border-gray-800 pb-4">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <CardTitle className="text-gray-800 dark:text-white">{t('generalSettings.title')}</CardTitle>
                        ...
                        <CardContent className="pt-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                                <div className="space-y-2">
                                    <Label htmlFor="templatesCount" className="text-gray-700 dark:text-gray-300 flex items-center gap-1">
                                ...
                                <div className="space-y-2">
                                    <Label htmlFor="questionsCount" className="text-gray-700 dark:text-gray-300 flex items-center gap-1">
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) => handleUpdateTotalQuestions(parseInt(e.target.value) || 1)}
                                        className="border-gray-300 dark:border-gray-700 focus:border-blue-500 dark:focus:border-blue-400"
                                    />
                                    <p className="text-xs text-gray-500 dark:text-gray-400">
                                        {t('generalSettings.available')}: {getTotalQuestionsInBank()} {t('questionsAvailable')}
                                    </p>
                                </div>

                                <div className="space-y-2">
                                    <Label htmlFor="totalMark" className="text-gray-700 dark:text-gray-300">{t('generalSettings.totalMark')}</Label>
                                    <Input
                                        id="totalMark"
                                        type="number"
                                        min="1"
                                ...
                                <div className="space-y-2">
                                    <Label htmlFor="totalTime" className="text-gray-700 dark:text-gray-300">{t('generalSettings.totalTime')}</Label>
                                    <Input
                                        id="totalTime"
                                        type="number"
                                        min="1"
                            ...
                            <div className="mb-6">
                                <h4 className="font-medium text-gray-700 dark:text-gray-300 mb-3 flex items-center gap-2">
                                    <span>{t('generalSettings.repeatLimits')}</span>
                                    <Badge variant="outline" className="text-xs">{t('generalSettings.optional')}</Badge>
                                </h4>
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                    <div className="space-y-2">
                                        <Label htmlFor="lastExamRepeatLimit1" className="text-gray-700 dark:text-gray-300">
                                            {t('generalSettings.repeatPreviousExam')}
                                        </Label>
                                        <Input
                                    ...
                                    <div className="space-y-2">
                                        <Label htmlFor="lastExamRepeatLimit2" className="text-gray-700 dark:text-gray-300">
                                            {t('generalSettings.repeatTwoExamsAgo')}
                                        </Label>
                                        <Input
                                    ...
                                    <div className="space-y-2">
                                        <Label htmlFor="lastExamRepeatLimit3" className="text-gray-700 dark:text-gray-300">
                                            {t('generalSettings.repeatThreeExamsAgo')}
                                        </Label>
                                        <Input
                                    ...
                                    <div className="space-y-2">
                                        <Label htmlFor="lastOthersExamRepeatLimit" className="text-gray-700 dark:text-gray-300">
                                            {t('generalSettings.repeatOtherExams')}
                                        </Label>
                                        <Input
                            ...
                            <div className="mt-4 space-y-3">
                                <Label htmlFor="instructions" className="text-gray-700 dark:text-gray-300">{t('generalSettings.examInstructions')}</Label>
                                <Textarea
                                    id="instructions"
                                    ...
                                    rows={2}
                                    className="border-gray-300 dark:border-gray-700 focus:border-blue-500 dark:focus:border-blue-400 resize-none"
                                />
                            </div>

                            <div className="mt-4 flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                                <div>
                                    <Label className="text-gray-700 dark:text-gray-300">{t('generalSettings.randomQuestions')}</Label>
                                    <p className="text-sm text-gray-500 dark:text-gray-400">
                                        {formData.randomQuestions ? t('generalSettings.randomDescription') : t('generalSettings.fixedDescription')}
                                    </p>
                                </div>
                                <Switch
                                    dir="ltr"
                        ...
                        <CardHeader className="border-b border-gray-100 dark:border-gray-800 pb-4">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <CardTitle className="text-gray-800 dark:text-white">{t('chaptersDistribution.title')}</CardTitle>
                                        <CardDescription className="text-gray-600 dark:text-gray-400">
                            ...
                            <div className="space-y-6">
                                <div>
                                    <div className="flex items-center justify-between mb-4">
                                        <Label className="text-gray-700 dark:text-gray-300">
                                            {t('chaptersDistribution.availableChapters')} ({availableChapters.length})
                                        </Label>
                                        <Badge variant="secondary" className="bg-gray-100 dark:bg-gray-800">
                                            {t('chaptersDistribution.totalQuestions')}: {getTotalQuestionsInBank()}
                                        </Badge>
                                    </div>
                                        ...
                                        <Table>
                                            <TableHeader className="bg-gray-50 dark:bg-gray-800">
                                                <TableRow>
                                                    ...
                                                    <TableHead className="text-right">{t('chaptersDistribution.chapter')}</TableHead>
                                                    <TableHead className="text-right">{t('chaptersDistribution.descriptionColumn')}</TableHead>
                                                    <TableHead className="text-center">{t('chaptersDistribution.availableQuestions')}</TableHead>
                                                    <TableHead className="text-center">{t('chaptersDistribution.action')}</TableHead>
                                                </TableRow>
                                            </TableHeader>
                                            <TableBody>
                                                {availableChapters.map((chapter, index) => {
                                                    const isSelected = formData.chapterSettings.some(s => s.chapterId === chapter.id);
                                                    const selectedSetting = formData.chapterSettings.find(s => s.chapterId === chapter.id);

                                                    return (
                                                        <TableRow
                                                            key={chapter.id}
                                                            className={`hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors ${isSelected ? 'bg-blue-50 dark:bg-blue-900/10' : ''}`}
                                                        >
                                                            ...
                                                            <TableCell>
                                                                <div className="text-sm text-gray-600 dark:text-gray-400 truncate max-w-[200px]">
                                                                    {chapter.description || t('chaptersDistribution.noDescription')}
                                                                </div>
                                                            </TableCell>
                                                                ...
                                                                {isSelected ? (
                                                                    <div className="flex items-center justify-center gap-2">
                                                                        <Input
                                                                            ...
                                                                            onChange={(e) => handleUpdateChapterCount(chapter.id, parseInt(e.target.value) || 0)}
                                                                            className="w-24 border-gray-300 dark:border-gray-700"
                                                                        />
                                                                        <Button
                                                                            size="sm"
                                                                            variant="ghost"
                                                                            onClick={() => handleRemoveChapterSetting(chapter.id)}
                                                                            className="h-8 w-8 p-0 text-rose-600 dark:text-rose-400 hover:text-rose-700 dark:hover:text-rose-300"
                                                                        >
                                                                    ...
                                                                    <Button
                                                                        size="sm"
                                                                        variant="outline"
                                                                        onClick={() => handleAddChapterSetting(chapter.id)}
                                                                        className="border-blue-500 dark:border-blue-400 text-blue-600 dark:text-blue-400 hover:bg-blue-600 dark:hover:bg-blue-700 hover:text-white"
                                                                    >
                                ...
                                {formData.chapterSettings.length > 0 && (
                                    <div className="bg-linear-to-r from-blue-50 to-blue-100/50 dark:from-blue-900/10 dark:to-blue-900/5 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                                        <h4 className="font-medium text-gray-800 dark:text-white mb-3">{t('chaptersDistribution.selectedChapters')}</h4>
                                        <div className="space-y-2">
                                            {formData.chapterSettings.map(setting => {
                                                ...
                                                const availableCount = chapter.questions?.length || 0;
                                                const percentage = availableCount > 0 ? (setting.count / availableCount) * 100 : 0;

                                                return (
                                                    <div key={setting.chapterId} className="flex items-center justify-between text-sm">
                                                        <div className="flex items-center gap-2">
                                                            <span className="font-medium text-gray-700 dark:text-gray-300">{chapter.title}</span>
                                                            <Badge variant="outline" className="text-xs">
                                                                {setting.count} {t('difficultyLevels.questions')}
                                                            </Badge>
                                                        </div>
                                                        <div className="flex items-center gap-3">
                                                            <div className="w-32">
                                                            ...
                                                            <span className="text-gray-500 dark:text-gray-400 text-xs">
                                                                {setting.count}/{availableCount} ({setting.percentage.toFixed(1)}%)
                                                            </span>
                                                        </div>
                                                    </div>
                                                );
                            ...
                            <CardHeader className="border-b border-gray-100 dark:border-gray-800 pb-4">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <CardTitle className="text-gray-800 dark:text-white">{t('difficultyLevels.title')}</CardTitle>
                                            <CardDescription className="text-gray-600 dark:text-gray-400">
                            ...
                            <CardContent className="pt-6">
                                <div className="space-y-4">
                                    {QUESTION_LEVELS.map(level => {
                                        const isSelected = formData.levelsSettings.some(s => s.level === level.id);
                                        const setting = formData.levelsSettings.find(s => s.level === level.id);
                                        const availableCount = levelStats[level.id]?.count || 0;

                                        return (
                                            <div key={level.id} className={`p-3 rounded-lg border transition-all ${isSelected ? 'border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-900/10' : 'border-gray-200 dark:border-gray-800'}`}>
                                                    ...
                                                    <div className="space-y-3">
                                                        <div className="flex items-center gap-2">
                                                            <Input
                                                                ...
                                                                onChange={(e) => handleUpdateLevelCount(level.id, parseInt(e.target.value) || 0)}
                                                                className="border-gray-300 dark:border-gray-700"
                                                            />
                                                            <Button
                                                                size="sm"
                                                                variant="ghost"
                                                                onClick={() => handleRemoveLevelSetting(level.id)}
                                                        ...
                                                        <div className="space-y-1">
                                                            <div className="flex justify-between text-xs">
                                                                <span className="text-gray-600 dark:text-gray-400">
                                                                    {setting?.percentage.toFixed(1)}% {t('difficultyLevels.totalPercentage')}
                                                                </span>
                            ...
                            <CardHeader className="border-b border-gray-100 dark:border-gray-800 pb-4">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <CardTitle className="text-gray-800 dark:text-white">{t('questionTypes.title')}</CardTitle>
                                            <CardDescription className="text-gray-600 dark:text-gray-400">
                            ...
                            <CardContent className="pt-6">
                                <div className="space-y-4">
                                    {availableQuestionTypes.map(type => {
                                        const isSelected = formData.typeSettings.some(s => s.questionTypeId === type.id);
                                        const setting = formData.typeSettings.find(s => s.questionTypeId === type.id);
                                        const availableCount = typeStats[type.id]?.count || 0;

                                        return (
                                            <div key={type.id} className={`p-3 rounded-lg border transition-all ${isSelected ? 'border-violet-300 dark:border-violet-700 bg-violet-50 dark:bg-violet-900/10' : 'border-gray-200 dark:border-gray-800'}`}>
                                                <div className="flex items-center justify-between mb-2">
                                                    <div className="flex items-center gap-2">
                                                    ...
                                                    <div className="space-y-3">
                                                        <div className="flex items-center gap-2">
                                                            <Input
                                                                ...
                                                                max={availableCount}
                                                                value={setting?.count || 0}
                                                                onChange={(e) => handleUpdateTypeCount(type.id, parseInt(e.target.value) || 0)}
                                                                className="border-gray-300 dark:border-gray-700"
                                                            />
                                                            <Button
                                                                size="sm"
                    ...
                    <Card className="border border-border-light dark:border-gray-800 bg-linear-to-r from-gray-50 to-white dark:from-gray-900 dark:to-gray-950 shadow-lg hover:shadow-xl transition-all duration-300">
                        <CardHeader className="border-b border-gray-100 dark:border-gray-800 pb-4">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <CardTitle className="text-gray-800 dark:text-white">{t('finalReview.title')}</CardTitle>
                                        <CardDescription className="text-gray-600 dark:text-gray-400">
                                ...
                                <Badge className={`${distributionSummary.isBalanced ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-700' : 'bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-300 border border-amber-300 dark:border-amber-700'}`}>
                                    {distributionSummary.isBalanced ? t('finalReview.ready') : t('finalReview.needsReviewBadge')}
                                </Badge>
                            </div>
                        </CardHeader>
                        <CardContent className="pt-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {/* Summary */}
                                <div className="space-y-4">
                                    <h4 className="font-medium text-gray-800 dark:text-white">{t('finalReview.templateSummary')}</h4>
                                    <div className="space-y-3">
                                        <div className="flex items-center justify-between p-2 hover:bg-gray-50 dark:hover:bg-gray-800/50 rounded">
                                            <span className="text-gray-600 dark:text-gray-400">{t('generalSettings.templatesCount')}</span>
                                            <span className="font-medium text-gray-800 dark:text-white">{formData.templatesCount}</span>
                                        </div>
                                        <Separator />
                                        <div className="flex items-center justify-between p-2 hover:bg-gray-50 dark:hover:bg-gray-800/50 rounded">
                                            <span className="text-gray-600 dark:text-gray-400">{t('generalSettings.totalQuestions')}</span>
                                            <span className="font-bold text-blue-600 dark:text-blue-400">{formData.questionsCount}</span>
                                        </div>
                                        <Separator />
                                        <div className="flex items-center justify-between p-2 hover:bg-gray-50 dark:hover:bg-gray-800/50 rounded">
                                            <span className="text-gray-600 dark:text-gray-400">{t('generalSettings.totalMark')}</span>
                                            <span className="font-medium text-gray-800 dark:text-white">{formData.totalMark}</span>
                                        </div>
                                        <Separator />
                                        <div className="flex items-center justify-between p-2 hover:bg-gray-50 dark:hover:bg-gray-800/50 rounded">
                                            <span className="text-gray-600 dark:text-gray-400">{t('generalSettings.totalTime')}</span>
                                            <span className="font-medium text-gray-800 dark:text-white">{formData.totalTime} {t('generalSettings.minutes')}</span>
                                        </div>
                                        <Separator />
                                        <div className="flex items-center justify-between p-2 hover:bg-gray-50 dark:hover:bg-gray-800/50 rounded">
                                            <span className="text-gray-600 dark:text-gray-400">{t('generalSettings.randomQuestions')}</span>
                                            <Badge variant={formData.randomQuestions ? "default" : "outline"}
                                                className={formData.randomQuestions ? "bg-blue-600 dark:bg-blue-700" : ""}
                                            >
                                                {formData.randomQuestions ? t('finalReview.enabled') : t('finalReview.disabled')}
                                            </Badge>
                                        </div>
                                        <div className="flex items-center justify-between p-2 hover:bg-gray-50 dark:hover:bg-gray-800/50 rounded">
                                            <span className="text-gray-600 dark:text-gray-400">{t('finalReview.repeatExam1')}</span>
                                            <span className="font-medium text-gray-800 dark:text-white">{formData.lastExamRepeatLimit1}</span>
                                        </div>
                                        <Separator />
                                        <div className="flex items-center justify-between p-2 hover:bg-gray-50 dark:hover:bg-gray-800/50 rounded">
                                            <span className="text-gray-600 dark:text-gray-400">{t('finalReview.repeatExam2')}</span>
                                            <span className="font-medium text-gray-800 dark:text-white">{formData.lastExamRepeatLimit2}</span>
                                        </div>
                                        <Separator />
                                        <div className="flex items-center justify-between p-2 hover:bg-gray-50 dark:hover:bg-gray-800/50 rounded">
                                            <span className="text-gray-600 dark:text-gray-400">{t('finalReview.repeatExam3')}</span>
                                            <span className="font-medium text-gray-800 dark:text-white">{formData.lastExamRepeatLimit3}</span>
                                        </div>
                                        <Separator />
                                        <div className="flex items-center justify-between p-2 hover:bg-gray-50 dark:hover:bg-gray-800/50 rounded">
                                            <span className="text-gray-600 dark:text-gray-400">{t('finalReview.repeatOther')}</span>
                                            <span className="font-medium text-gray-800 dark:text-white">{formData.lastOthersExamRepeatLimit}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                    <h4 className="font-medium text-gray-800 dark:text-white">{t('finalReview.distributionPreview')}</h4>
                                    <div className="space-y-3">
                                        <div className="p-3 bg-blue-50 dark:bg-blue-900/10 rounded-lg">
                                            <div className="flex justify-between items-center mb-2">
                                                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('finalReview.chaptersPreview')}</span>
                                                <Badge variant="outline">{formData.chapterSettings.length}</Badge>
                                            </div>
                                            <div className="text-xs text-gray-600 dark:text-gray-400 space-y-1">
                                                {formData.chapterSettings.slice(0, 3).map(setting => {
                                                    const chapter = getSelectedChapter(setting.chapterId);
                                                    return chapter ? (
                                                        <div key={setting.chapterId} className="flex justify-between">
                                                            <span className="truncate">{chapter.title}</span>
                                                            <span>{setting.count} {t('difficultyLevels.questions')}</span>
                                                        </div>
                                                    ) : null;
                                                })}
                                                {formData.chapterSettings.length > 3 && (
                                                    <div className="text-center pt-1 text-gray-500">
                                                        {t('finalReview.andMore', { count: formData.chapterSettings.length - 3 })}
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        {formData.levelsSettings.length > 0 && (
                                            <div className="p-3 bg-amber-50 dark:bg-amber-900/10 rounded-lg">
                                                <div className="flex justify-between items-center mb-2">
                                                    <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('finalReview.difficultyPreview')}</span>
                                                    <Badge variant="outline">{formData.levelsSettings.length}</Badge>
                                                </div>
                                                <div className="flex gap-2">
                                                    {formData.levelsSettings.map(setting => {
                                                        const level = QUESTION_LEVELS.find(l => l.id === setting.level);
                                                        return level ? (
                                                            <Badge key={setting.level} className={level.color}>
                                        ...
                                        {formData.typeSettings.length > 0 && (
                                            <div className="p-3 bg-violet-50 dark:bg-violet-900/10 rounded-lg">
                                                <div className="flex justify-between items-center mb-2">
                                                    <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('finalReview.typesPreview')}</span>
                                                    <Badge variant="outline">{formData.typeSettings.length}</Badge>
                                                </div>
                                                <div className="flex flex-wrap gap-2">
                                                    {formData.typeSettings.map(setting => {
                                                        const type = availableQuestionTypes.find(t => t.id === setting.questionTypeId);
                                                        return type ? (
                                                            <Badge key={setting.questionTypeId} className={getQuestionTypeColor(type.code)}>
                                                                {getQuestionTypeDisplayName(type.code)}: {setting.count}
                                                            </Badge>
                                                        ) : null;
                                                    })}
                                                </div>
                        ...
                        <CardFooter className="border-t border-gray-100 dark:border-gray-800 pt-4">
                            <Button
                                ...
                                disabled={isSaving || validationErrors.length > 0 || formData.questionsCount === 0 || formData.chapterSettings.length === 0}
                            ...
                            >
                                {isSaving ? (
                                    ...
                                    <>
                                        ...
                                        {t('saveTemplate')} ({formData.templatesCount} {t('generalSettings.templates')})
                                    </>
                                )}
                            </Button>
                        </CardFooter>

...
export default function BankTemplatesPage() {
    ...
    const bankData = useSelector((state: RootState) => state.questions.bankTemplates);
    ...
    useEffect(() => {
        ...
        try {
            ...
            toast.error(error instanceof Error ? error.message : t('errors.loadTemplatesFailed'));
        } finally {
            setLoading(false);
        }
    ...
    const groupTemplatesByExactTime = (): TemplateGroup[] => {
        ...
        const groups: TemplateGroup[] = Array.from(timeMap.entries())
            ...
            .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()); // ترتيب تنازلي حسب التاريخ
            ...
            .map(group => {
                const filteredTemplates = group.templates.filter(template => {
                    const matchesSearch = template.code.toLowerCase().includes(searchTerm.toLowerCase()) ||
                        ...
                        group.dateFormatted.toLowerCase().includes(searchTerm.toLowerCase()) ||
                        group.timeFormatted.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    const handleDeleteTemplate = async () => {
        ...
        try {
            ...
            toast.error(error instanceof Error ? error.message : t('errors.deleteTemplateFailed'));
        }
    };
    ...
    const calculateStats = () => {
        ...
        const totalQuestions = bankData.templates.reduce((sum, t) => sum + t.questionsCount, 0);
        const avgTime = bankData.templates.reduce((sum, t) => sum + t.totalTime, 0) / total || 0;
        const avgMark = bankData.templates.reduce((sum, t) => sum + t.totalMark, 0) / total || 0;
        const groups = groupTemplatesByExactTime();
    ...
    return (
        <div className="min-h-screen bg-linear-to-b from-gray-50 to-white dark:from-gray-950 dark:to-gray-900" dir="rtl">
            <div className="max-w-7xl mx-auto px-4 py-6">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <div className="flex items-center gap-3 mb-1">
                                    <h1 className="text-2xl font-bold text-gray-800 dark:text-white">{bankData?.code}</h1>
                                    <Badge
                                        className={bankData?.isActive
                                    ...
                                    >
                                        {bankData?.isActive ? t('stats.active') : t('stats.inactive')}
                                    </Badge>
                                </div>
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            <Button
                                ...
                                className="bg-blue-600 hover:bg-blue-700 dark:bg-blue-700 dark:hover:bg-blue-800 text-white transition-colors shadow-md hover:shadow-lg"
                            >
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                            ...
                            <CardContent className="pt-6">
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <div className="text-sm text-gray-600 dark:text-gray-400">{t('stats.totalTemplates')}</div>
                                    </div>
                        ...
                        <Card className="border border-border-light dark:border-gray-800 bg-white dark:bg-gray-900 hover:shadow-lg transition-all duration-300">
                            <CardContent className="pt-6">
                                <div className="flex items-center justify-between">
                                    <div>
                                        <div className="text-2xl font-bold text-gray-800 dark:text-white">{stats.totalQuestions}</div>
                                        <div className="text-sm text-gray-600 dark:text-gray-400">{t('stats.totalQuestions')}</div>
                                    </div>
                                    <div className="p-2 bg-emerald-100 dark:bg-emerald-900/30 rounded-lg">
                        ...
                        <Card className="border border-border-light dark:border-gray-800 bg-white dark:bg-gray-900 hover:shadow-lg transition-all duration-300">
                            <CardContent className="pt-6">
                                <div className="flex items-center justify-between">
                                    <div>
                                        <div className="text-2xl font-bold text-gray-800 dark:text-white">{stats.uniqueTimes}</div>
                                        <div className="text-sm text-gray-600 dark:text-gray-400">{t('stats.creationBatches')}</div>
                                    </div>
                        ...
                        <Card className="border border-border-light dark:border-gray-800 bg-white dark:bg-gray-900 hover:shadow-lg transition-all duration-300">
                            <CardContent className="pt-6">
                                <div className="flex items-center justify-between">
                                    <div>
                                        <div className="text-2xl font-bold text-gray-800 dark:text-white">{stats.avgTemplatesPerBatch.toFixed(1)}</div>
                                        <div className="text-sm text-gray-600 dark:text-gray-400">{t('stats.averagePerBatch')}</div>
                                    </div>
                                    <div className="p-2 bg-emerald-100 dark:bg-emerald-900/30 rounded-lg">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2">
                        <Card className="border border-border-light dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm">
                            <CardHeader>
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                    <div>
                                        <CardTitle className="text-gray-800 dark:text-white">{t('title')}</CardTitle>
                                    </div>
                                    <div className="relative w-full sm:w-auto">
                                        ...
                                        <Input
                                            ...
                                            className="pr-10 border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-800 dark:text-white focus:border-blue-500 dark:focus:border-blue-400 focus:ring-2 focus:ring-blue-200 dark:focus:ring-blue-900/30 transition-colors w-full"
                                            value={searchTerm}
                            ...
                            <CardContent>
                                ...
                                <div className="space-y-4">
                                    {groups.map(group => {
                                        ...
                                        return (
                                            <div key={group.createdAt} className="border border-gray-200 dark:border-gray-800 rounded-lg overflow-hidden">
                                                {/* Group Header */}
                                                <div
                                                            ...
                                                            <div>
                                                                ...
                                                                <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                                                                    <span className="flex items-center gap-1">
                                                                        ...
                                                                        {group.templatesCount} {t('group.templates')}
                                                                    </span>
                                                                </p>
                                                            </div>
                                                        </div>
                                                                ...
                                                                {group.templates.map(template => (
                                                                    <tr key={template.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                                                        <td className="py-3 px-4 border-b border-gray-100 dark:border-gray-800">
                                                                            <div className="font-medium text-gray-800 dark:text-white">{template.code}</div>
                                                                        </td>
                                                                        <td className="py-3 px-4 border-b border-gray-100 dark:border-gray-800">
                                                                            <div className="text-sm text-gray-600 dark:text-gray-400 max-w-xs truncate">
                                                                                {template.instructions || t('table.noInstructions')}
                                                                            </div>
                                                                        </td>
                                                                        <td className="py-3 px-4 border-b border-gray-100 dark:border-gray-800">
                                                                        ...
                                                                        <td className="py-3 px-4 border-b border-gray-100 dark:border-gray-800">
                                                                            <Badge className="bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300 border border-blue-200 dark:border-blue-700">
                                                                                {template.totalTime} {t('previewDialog.minutes')}
                                                                            </Badge>
                                                                        </td>
                                                                        <td className="py-3 px-4 border-b border-gray-100 dark:border-gray-800">
                                                                            <div className="flex items-center gap-2">
                                                                                ...
                                                                                >
                                                                                    <Link href={`/questions_bank/${id}/${bankId}/bankTemplate/${template.id}`} className="flex items-center">
                    ...
                    <div className="space-y-6">
                            ...
                            <CardContent className="space-y-4">
                                <div className="space-y-3">
                                    <div className="flex justify-between items-center">
                                        <span className="text-sm text-gray-600 dark:text-gray-400">{t('stats.bankCode')}</span>
                                        <span className="font-medium text-gray-800 dark:text-white">{bankData?.code}</span>
                                    </div>
                                    <Separator className="bg-gray-200 dark:bg-gray-700" />
                                    <div className="flex justify-between items-center">
                                        <span className="text-sm text-gray-600 dark:text-gray-400">{t('stats.course')}</span>
                                        <span className="font-medium text-gray-800 dark:text-white text-left">{bankData?.course}</span>
                                    </div>
                                    <Separator className="bg-gray-200 dark:bg-gray-700" />
                                    <div className="flex justify-between items-center">
                                        <span className="text-sm text-gray-600 dark:text-gray-400">{t('stats.courseId')}</span>
                                        <span className="font-medium text-gray-800 dark:text-white">{bankData?.courseId}</span>
                                    </div>
                                    <Separator className="bg-gray-200 dark:bg-gray-700" />
                                    <div className="flex justify-between items-center">
                                        <span className="text-sm text-gray-600 dark:text-gray-400">{t('stats.supervisor')}</span>
                                        <span className="font-medium text-gray-800 dark:text-white">{bankData?.supervisor}</span>
                                    </div>
                                    <Separator className="bg-gray-200 dark:bg-gray-700" />
                                    <div className="flex justify-between items-center">
                                        <span className="text-sm text-gray-600 dark:text-gray-400">{t('stats.creationDate')}</span>
                                        <span className="font-medium text-gray-800 dark:text-white">{formatDate(bankData?.createdAt)}</span>
                                    </div>
                                    <Separator className="bg-gray-200 dark:bg-gray-700" />
                                    <div className="flex justify-between items-center">
                                        <span className="text-sm text-gray-600 dark:text-gray-400">{t('stats.status')}</span>
                                        <Badge
                                            className={bankData?.isActive
                                                ? "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-700"
                        ...
                        <Card className="border border-border-light dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm">
                            <CardHeader>
                                <CardTitle className="text-gray-800 dark:text-white">{t('quickActions')}</CardTitle>
                            </CardHeader>
            ...
            <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
                <DialogContent className="sm:max-w-[425px] border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900" dir="rtl">
                    <DialogHeader>
                        <DialogTitle className="text-gray-800 dark:text-white">
                            {t('deleteDialog.title')}
                        </DialogTitle>
                        <DialogDescription className="text-gray-600 dark:text-gray-400">
                            {t('deleteDialog.description', { code: selectedTemplate?.code || '' })}
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <DialogFooter>
                        <Button
                            ...
                            className="border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 hover:border-blue-500 dark:hover:border-blue-400"
                        >
            ...
            <Dialog open={isPreviewDialogOpen} onOpenChange={setIsPreviewDialogOpen}>
                <DialogContent className="sm:max-w-[600px] border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900" dir="rtl">
                    <DialogHeader>
                        <DialogTitle className="text-gray-800 dark:text-white">
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
                                        <p className="text-sm text-gray-800 dark:text-gray-300">{selectedTemplate.instructions || t('table.noInstructions')}</p>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-3 gap-4">
                                    <Card className="border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
                                        <CardContent className="pt-6">
                                            <div className="text-center">
                                                <div className="text-2xl font-bold text-gray-800 dark:text-white">{selectedTemplate.questionsCount}</div>
                                                <div className="text-sm text-gray-600 dark:text-gray-400">{t('previewDialog.questions')}</div>
                                            </div>
                                        </CardContent>
                                    </Card>
                                    <Card className="border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
                                        <CardContent className="pt-6">
                                            <div className="text-center">
                                                <div className="text-2xl font-bold text-gray-800 dark:text-white">{selectedTemplate.totalMark}</div>
                                                <div className="text-sm text-gray-600 dark:text-gray-400">{t('previewDialog.totalMark')}</div>
                                            </div>
                                        </CardContent>
                                    </Card>
                                    <Card className="border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
                                        <CardContent className="pt-6">
                                            <div className="text-center">
                                                <div className="text-2xl font-bold text-gray-800 dark:text-white">{selectedTemplate.totalTime}</div>
                                                <div className="text-sm text-gray-600 dark:text-gray-400">{t('previewDialog.minutes')}</div>
                                            </div>
                                        </CardContent>
                                    </Card>
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="grid grid-cols-2 gap-4 text-sm">
                                        <div>
                                            <span className="text-gray-600 dark:text-gray-400">{t('previewDialog.templateId')}</span>
                                            <span className="font-medium text-gray-800 dark:text-white mr-2">{selectedTemplate.id}</span>
                                        </div>
                                        <div>
                                            <span className="text-gray-600 dark:text-gray-400">{t('previewDialog.creationDate')}</span>
                                            <span className="font-medium text-gray-800 dark:text-white mr-2">{formatDate(selectedTemplate.createdAt)}</span>
                                        </div>
                                        <div>
                                            <span className="text-gray-600 dark:text-gray-400">{t('previewDialog.code')}</span>
                                            <span className="font-medium text-gray-800 dark:text-white mr-2">{selectedTemplate.code}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>

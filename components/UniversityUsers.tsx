...
import { searchUsers, assignUserToUnivsersity } from "@/store/user";
...
const UniversityUsers = ({
  ...
  const handleAssignUser = async (userId: number) => {
    ...
    try {
      const result = await dispatch(
        assignUserToUnivsersity({
          userId,
          universityId,
        }),
      ).unwrap();

      toast.success(result.message || "تم إضافة المستخدم للجامعة بنجاح");
      ...
      toast.error(
        error instanceof Error ? error.message : "فشل إضافة المستخدم للجامعة",
      );
    } finally {
      setAssigningUserId(null);
    }
  ...
  // Check if a user is already in the university
  const isUserInUniversity = (userId: number) => {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      ...
      >
        ...
        <DialogHeader className="p-6 border-b border-gray-200 dark:border-gray-800">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <DialogTitle className="text-2xl font-bold text-prim dark:text-sec flex items-center gap-2">
                  مستخدمي الجامعة
                  <Badge
                    variant="secondary"
                    className="mr-2 bg-sec text-white font-bold"
                  >
                    {users.length}
                  </Badge>
                </DialogTitle>
                <DialogDescription className="mt-1">
                  <TextMuted>
                    إدارة المستخدمين المرتبطين بـ {universityName}
                  </TextMuted>
                </DialogDescription>
              </div>
            </div>
                ...
                {(
                  ...
                  return (
                    <Button
                      ...
                      className={
                        isActive
                          ? "bg-sec text-white hover:opacity-80"
                          : "  border-1 border-sec dark:border-sec text-gray-800 dark:text-gray-300 hover:scale-[1.05]"
                      }
                    >
        ...
        <div className="p-6 max-h-[calc(90vh-320px)] overflow-y-auto custom-scrollbar">
          {/* No search yet - Show current university users */}
          {!hasSearched && activeFields.length === 0 && (
            <>
              {users.length > 0 ? (
                <>
                  <div className="mb-4">
                    <h3 className="text-lg font-semibold text-prim dark:text-sec">
                      المستخدمين الحاليين
                    </h3>
                  </div>
                ...
                <div className="text-center py-16">
                  ...
                  <p className="text-gray-600 dark:text-gray-400 max-w-md mx-auto">
                    لم يتم العثور على أي مستخدمين مرتبطين بهذه الجامعة
                  </p>
              ...
              {searchResults.length > 0 ? (
                <div className="space-y-3">
                  {searchResults.map((user: SearchedUser) => {
                    const inUniversity = isUserInUniversity(user.id);
                    return (
          ...
          <div className="flex items-center justify-between">
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {hasSearched
                ...
                : `عرض ${users.length} مستخدم في الجامعة`}
            </p>
            <Button
              variant="outline"
              onClick={onClose}
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {user.email && (
            <div className="flex items-center gap-1.5 text-sm text-gray-700 dark:text-gray-300">
              <Mail className="text-sec h-3.5 w-3.5" />
          ...
          {user.mobile && (
            <div className="flex items-center gap-1.5 text-sm text-gray-700 dark:text-gray-300">
              <Phone className="text-sec h-3.5 w-3.5" />
          ...
          {user.createDate && (
            <div className="flex items-center gap-1.5 text-sm text-gray-700 dark:text-gray-300">
              <Calendar className="text-sec h-3.5 w-3.5" />
